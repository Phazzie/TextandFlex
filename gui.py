...
class FileConverterApp:
    def __init__(self, root):
        ...
        style.configure('TLabelframe.Label', background=COLORS["background"], foreground=COLORS["text"], font=('Segoe UI', 10, 'bold'))
        style.configure('TButton', background=COLORS["primary"], foreground='white', font=('Segoe UI', 9))
        ...
        style.configure('TEntry', background='white', font=('Segoe UI', 9))
        ...
        self.use_project_dir = tk.BooleanVar(value=False)
        ...
        style.configure('Convert.TButton',
                      ...
                      padding=10)
        style.map('Convert.TButton', background=[('active', COLORS["primary"])])
    ...
    def toggle_output_dir(self):
        ...
        if self.use_project_dir.get():
            ...
            self.project_dir_label.configure(foreground=COLORS["primary"], font=('Segoe UI', 9, 'bold'))
        else:
            # Using custom directory, enable controls
            for child in self.custom_dir_frame.winfo_children():
                child.configure(state="normal")
            self.project_dir_label.configure(foreground=COLORS["secondary"], font=('Segoe UI', 9))
